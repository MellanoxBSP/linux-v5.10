 ...
 *
 * Copyright (C) 2021 Nvidia Technologies Ltd.
 */
...
#define NVSW_SN2201_DEFER_TIME	(msecs_to_jiffies(10))
#define NVSW_SN2201_DEFER_COUNT	20
 ...
 * @main_mux_dwork: delayed work for the devices which connected to main mux;
 * @main_mux_dwork_counter: delayed work re-try counter;
...
struct nvsw_sn2201 {
	...
	int cpld_devs_num;
	struct delayed_work main_mux_dwork;
	int main_mux_dwork_counter;
	int main_mux_deferred_nr;
};

struct nvsw_sn2201_kobj {
	const char *kname;
	struct kobj_type ktype;
	struct kobject kobj;
};

static struct nvsw_sn2201_kobj nvsw_sn2201_kobjs[] = {
	{
		.kname = "nvsw_sn2201_fan",
	},
};
...
static bool nvsw_sn2201_writeable_reg(struct device *dev, unsigned int reg)
{
	...
	case NVSW_SN2201_FRONT_FAN_LED_CTRL_OFFSET:
...
static struct property nvsw_sn2201_fan_pwm_max_prop = {
	.name = "emc2305,max-pwm",
	.length = 4,
	.value = "255",
};

static struct property nvsw_sn2201_fan_pwm_min_prop = {
	.name = "emc2305,min-pwm",
	.length = 2,
	.value = "0",
	.next = &nvsw_sn2201_fan_pwm_max_prop,
};

static struct property nvsw_sn2201_fan_cooling_levels_prop = {
	.name = "emc2305,cooling-levels",
	.length = 3,
	.value = "10",
	.next = &nvsw_sn2201_fan_pwm_min_prop,
};

static struct device_node nvsw_sn2201_fan_device_node = {
	.name = "emc2305",
	.properties = &nvsw_sn2201_fan_cooling_levels_prop,
};
...
static struct i2c_board_info nvsw_sn2201_static_devices[] = {
	...
	},
	{
		...
		.of_node =  &nvsw_sn2201_fan_device_node,
	},
	{
...
static struct mlxreg_core_data nvsw_sn2201_led_data[] = {
	...
	{
		.label = "fan:green",
		.reg = NVSW_SN2201_FRONT_FAN_LED_CTRL_OFFSET,
		.mask = GENMASK(7, 4),
	},
	{
		.label = "fan:orange",
...
static struct mlxreg_core_data nvsw_sn2201_io_data[] = {
	...
	{
		.label = "cpld1_pn",
		.reg = NVSW_SN2201_CPLD_ID_OFFSET,
		.bit = GENMASK(7, 0),
		.mode = 0444,
	},
	{
				  ...
				  int size)
{
	...
	/* Create I2C device for SN2201 static devices. */
...
static void nvsw_sn2201_main_mux_work(struct work_struct *work)
{
	...
	nvsw_sn2201 = container_of(work, struct nvsw_sn2201, main_mux_dwork.work);
	dev = nvsw_sn2201->dev;
	...
	/* Create SN2201 static devices. */
	nvsw_sn2201->sn2201_devs = devm_kmemdup(dev, nvsw_sn2201_static_brdinfo,
						sizeof(nvsw_sn2201_static_brdinfo), GFP_KERNEL);
	if (!nvsw_sn2201->sn2201_devs) {
		dev_err(dev, "Failed to allocate memory for static devices\n");
		return;
	}
	nvsw_sn2201->sn2201_devs_num = ARRAY_SIZE(nvsw_sn2201_static_brdinfo);

	/* Update I2C devices adapter. */
	sn2201_dev = nvsw_sn2201->sn2201_devs;
	for (i = 0, j = 0; i < nvsw_sn2201->sn2201_devs_num; i++, sn2201_dev++) {
		if (sn2201_dev->brdinfo->of_node)
			sn2201_dev->brdinfo->of_node->kobj = nvsw_sn2201_kobjs[j++].kobj;
...
static int nvsw_sn2201_i2c_complition_notify(void *handle, int id)
{
	struct nvsw_sn2201 *nvsw_sn2201 = handle;
	void *regmap;
	int i, err;

	/* Create main mux first. */
	nvsw_sn2201->main_mux_deferred_nr = NVSW_SN2201_MAIN_MUX_DEFER_NR;
	nvsw_sn2201->main_mux_devs = devm_kmemdup(nvsw_sn2201->dev, nvsw_sn2201_main_mux_brdinfo,
						  sizeof(nvsw_sn2201_main_mux_brdinfo), GFP_KERNEL);
	if (!nvsw_sn2201->main_mux_devs) {
		dev_err(nvsw_sn2201->dev, "Failed to allocate memory for main mux device\n");
		return -ENOMEM;
	}
	nvsw_sn2201->cpld_devs = devm_kmemdup(nvsw_sn2201->dev, nvsw_sn2201_cpld_brdinfo,
					      sizeof(nvsw_sn2201_cpld_brdinfo), GFP_KERNEL);
	if (!nvsw_sn2201->cpld_devs) {
		dev_err(nvsw_sn2201->dev, "Failed to allocate memory for cpld device\n");
		return -ENOMEM;
	}
	...
	/* Create SN2201 static devices. */
	INIT_DELAYED_WORK(&nvsw_sn2201->main_mux_dwork, nvsw_sn2201_main_mux_work);
	schedule_delayed_work(&nvsw_sn2201->main_mux_dwork, NVSW_SN2201_DEFER_TIME);
...
static int nvsw_sn2201_config_pre_init(struct nvsw_sn2201 *nvsw_sn2201)
{
	...
	nvsw_sn2201->i2c_data->completion_notify = nvsw_sn2201_i2c_complition_notify;
...
static void
nvsw_sn2201_config_post_exit(struct nvsw_sn2201 *nvsw_sn2201)
{
...
static int nvsw_sn2201_kobject_init(struct device *dev)
{
	int i, err;

	for (i = 0; i < ARRAY_SIZE(nvsw_sn2201_kobjs); i++) {
		err = kobject_init_and_add(&nvsw_sn2201_kobjs[i].kobj,
					   &nvsw_sn2201_kobjs[i].ktype,
					   NULL, nvsw_sn2201_kobjs[i].kname);
		if (err) {
			dev_err(dev, "Failed to initialize kobject %s\n",
				nvsw_sn2201_kobjs[i].kname);
...
kobject_init_and_add_fail:
	for (i = i - 1; i >= 0; i--) {
		kobject_del(&nvsw_sn2201_kobjs[i].kobj);
		kobject_put(&nvsw_sn2201_kobjs[i].kobj);
	}
	return err;
}

static void nvsw_sn2201_kobject_exit(void)
{
	int i;

	for (i = ARRAY_SIZE(nvsw_sn2201_kobjs) - 1; i >= 0; i++) {
		kobject_del(&nvsw_sn2201_kobjs[i].kobj);
		kobject_put(&nvsw_sn2201_kobjs[i].kobj);
	}
}
...
static int nvsw_sn2201_probe(struct platform_device *pdev)
{
	...
	err = nvsw_sn2201_kobject_init(&pdev->dev);
	if (err)
		return err;
	...
	err = nvsw_sn2201_config_pre_init(nvsw_sn2201);
	if (err)
		goto nvsw_sn2201_config_pre_init_fail;

	return 0;

nvsw_sn2201_config_pre_init_fail:
devm_kzalloc_fail:
	nvsw_sn2201_kobject_exit();
	return err;
}
...
static int nvsw_sn2201_remove(struct platform_device *pdev)
{
	...
	nvsw_sn2201_config_post_exit(nvsw_sn2201);
	nvsw_sn2201_kobject_exit();
